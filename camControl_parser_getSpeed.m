function [ value ] = camControl_parser_getSpeed( commands )
%CAMCONTROL_PARSER_GETSPEED Extrae la velocidad de la salida de los 
%comandos
% Analiza las respuestas de lasordenes previamente ejecutadas y obtiene 
% el valor de la velocidad cuando se ha solicitado la obtención del valor de
% dicho parámetro previamente.
% Por ejemplo, si antes se ha solicitado el valor de la velocidad, luego se
...
for i=fliplr(1:size(commands,2)),       %Start searching for the end
    if strcmp(commands(i).command,'get') && strcmp(commands(i).parameter,'SPEED')
